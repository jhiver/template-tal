...
function tal_on_error (node, xml, end, context, callback) {
  ...
  tal_define (node, xml, end, context, function(err, result){
    ...
    resolve_expression(stuff, context, function(error, result){
      if(error) { console.error(error) }
      result = result || ''
      result.push(result)
      result.push (end);
...
function tal_repeat_each (array, node, xml, end, context, callback, count, temp) {
  ...
  tal_content(newNode, shallowCopy(xml), end, newContext, function (error, result) {
    ...
    result = result || ''
    temp.push(result)
    return tal_repeat_each(array, node, xml, end, context, callback, count, temp)
  })
}
...
function tal_repeat (node, xml, end, context, callback) {
  ...
  resolve_expression(expression, context, function(error, array) {
    if(error) { console.error(error) }
    array = array || [];
    tal_repeat_each (array, node, xml, end, context, function (error, result){
      if(error) { console.error(error) }
      result = result || []
      return callback(null, result.join(''))
    })
...
function tal_attributes (node, xml, end, context, callback) {
  ...
  var array = stuff.split(/;(?!;)/)
  tal_attributes_each(array, array, node, xml, end, context, function(){
...
function tal_omit_tag (node, xml, end, context, callback) {
  ...
  if (stuff == null) {
    ...
    if (stuff == "") {
      ...
      omit = resolve_expression (stuff, context, callback);
    }
  }
...
function resolve (expr, self, callback) {
  ...
  for (var mod in MODIFIERS) {
    ...
    if (expr.match (regex)) {
      ...
      return MODIFIERS[mod] (expr, self, callback)
    }
  }
  ...
  try {
    ...
    if ((typeof(err) == "string") && err.match(/is not defined/)) { return callback (null, null) }
    self.ERROR = err
