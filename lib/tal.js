...
function process (xml, context) {
	...
	while(xml.length) {
		...
		if (opentag) {
			...
			var balance = 1;
			while (balance) {
                if (xml.length == 0) { throw ("cannot find closing tag for " + elem); }
				var elem = xml.shift();
...
function tal_define (node, xml, end, context) {
    ...
    var newContext = deepCopy(context);
...
function tal_repeat (node, xml, end, context) {
    ...
    for (var i=0; i<array.length; i++) {
    	...
    	result.push(tal_content(newNode, xml, end, newContext));
    }
    return result.join('');
}
...
function resolve (expr, self) {
    ...
    try {
        ...
        if(err.match(/is not defined/)) { return null };
        throw err;
    }
}
...
function node2tag (node) {
    ...
    var open  = delete node['_open'];
    var close = delete node['_close'];
